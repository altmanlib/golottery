...
| 宾客入场 | 扫活动码 / 打开活动短链，绑定到指定 `event` | 同一微信可参加多场；活动之间数据隔离 |
...
2. 创建活动（成功即扣 1 场次），填写名称与签到时间窗。
3. 导入参会名单（姓名 + 手机号后四位，可选部门）；在地图上选定中心点与半径。
4. 录入奖项与抽取顺序；添加现场工作人员。
5. 下载活动码；用测试账号跑通签到与抽奖。
...
宾客首次进入需匹配该活动名单；之后再打开同一活动直接显示签到状态。位置不在范围内或定位失败时，可提交人工确认，不会被直接永久拒绝。
...
| O4 | 活动码与短链 | 生成进入该活动的小程序码与短链，可下载 | P0 |
...
| G2 | 定位签到 | 点击签到后获取定位，服务端校验围栏，成功即入池 | P0 |
| G3 | 签到状态 | 展示未签到、已签到、待人工确认及签到时间 | P0 |
| G4 | 定位授权引导 | 拒绝授权时说明原因并引导打开设置 | P0 |
| G5 | 人工确认申请 | 定位失败或不在范围内时可提交申请并查看处理结果 | P0 |
...
| M3 | 人工确认 | 查看待确认申请，通过或拒绝，并记录操作人 | P0 |
| M4 | 代为签到 | 按姓名搜索宾客并直接代为签到 | P1 |
| M5 | 围栏设置 | 用 `wx.chooseLocation` 选中心点，输入半径，保存后立即生效 | P0 |
...
| 编号 | 功能 | 说明 | 优先级 |
| --- | --- | --- | --- |
| B1 | 名单导入 | 上传 Excel，校验重复与格式错误，展示导入结果 | P0 |
| B2 | 围栏与时间窗 | 配置中心点、半径、签到起止时间 | P0 |
...
| 活动进入 | 无活动码/短链上下文时，不能浏览或签到其他活动 |
| 控制台权限 | 组织管理员可管理本组织全部活动；不可见其他组织 |
| 小程序管理入口 | 仅当该用户在该活动下具有 staff/admin 授权时可见 |
| 大屏权限 | 活动级 host 凭证；SSE 与抽奖接口校验活动归属 |
| 场次配额 | 创建活动消耗 1 个场次（或按套餐定义）；配额用尽不可创建 |
| 人数档位 | 单场名单导入上限由套餐约束，超出拒绝导入 |

## 7. 定位签到规则

签到是否通过完全由服务端判定：小程序只上报坐标和精度，服务端用距离减去定位精度后与半径比较。
...
| 人工确认 | 工作人员通过后视为已签到；导出标注「人工确认」与操作人 |
...
| 隐私合规 | 小程序隐私指引声明收集位置与姓名；仅在用户点击签到时取一次定位 |
...
| 活动 | 所属组织、名称、中心点、半径、签到时间窗、是否允许兼中、状态 |
| 参会人员 | 活动、姓名、部门、手机号后四位、绑定 openid、签到状态、签到时间、签到方式 |
| 签到尝试 | 活动、人员、openid、经纬度、精度、距离、结果、失败原因、时间 |
| 人工确认申请 | 人员、原因、状态、处理人、处理时间 |
| 奖项 | 活动、名称、奖品、人数、顺序 |
...
已决产品规则见 [decisions.md](decisions.md)。
...
| 租户数据串扰 | 安全与合规事故 | 行级隔离强制中间件、越权测试、审计日志 |
...
| 阶段 | 事项 | 产出 |
| --- | --- | --- |
| M0 | 需求与套餐草案确认，备案与定位权限启动 | 定稿 PRD、主体与域名就绪计划 |
| M1 | 活动引擎：签到、抽奖、大屏、导入导出 | 可支撑单组织多活动的测试版 |
| M2 | SaaS 壳：组织、配额、活动码、控制台 | 可手动开通并售卖的 MVP |
| M3 | 售前试跑与压测 | 体验档客户跑通；签到压测达标 |
| M4 | 首场付费活动现场值守 | 完整签到与抽奖；复盘清单 |
