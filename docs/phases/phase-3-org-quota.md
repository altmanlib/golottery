...
运营登录后可以开通组织，并手工增减场次配额。这是活动数据的租户边界：后续活动、名单、奖项都挂 `org_id`，活动首次就绪时再扣场次。
...
| 配额规则 | 活动首次进入 `ready` 时消耗 1 个场次；用尽后不能就绪。扣减动作不在本模块 |
| 人数档位 | 体验 100、标准 800、加量 2000。首发由运营写入，不在线购买 |
| 人数上限粒度 | `org_quotas.max_attendees` 是组织默认值。创建活动时复制到 `events.max_attendees`，运营可单独调高某场活动（[阶段 5](phase-5-event-setup.md)）。不按档位分账；上线自助购买时再改为按档位分账 |
| 隔离 | 共享库，业务行带 `org_id`。本模块的行本身就是组织 |

人数上限在这里只保存。调整组织默认值只影响之后创建的活动。
...
| `event_id` | uuid | 可空；活动就绪扣场次时填写，外键在活动迁移中补加 |
...
- 活动就绪时扣场次。活动模块消费 `event_credits`，并再写一条负数流水
- 删除组织
- 修改历史流水
- 按人数档位自动换算场次价格
- 跨组织汇总报表
